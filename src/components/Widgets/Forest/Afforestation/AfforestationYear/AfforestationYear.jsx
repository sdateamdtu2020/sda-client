...
import { useStyles } from "./AfforestationYear.elements";
...
const AfforestationYear = ({ id, data, inputs, outputs }) => {
	...
	return (
		<div className={classes.node}>
			...
			<div className={classes.body}>
				...
				>
					<TreeItem
						nodeId={`${treeAfforestation.id}-${treeAfforestation.year.id}`}
						label={treeAfforestation.year.name}
					>
						{treeAfforestation.year.children.map((item) => (
							<TreeItem
								nodeId={`${treeAfforestation.id}-${treeAfforestation.year.id}-${item.id}`}
								label={item.name}
								key={item.id}
							/>
						))}
...
export default AfforestationYear;
