...
import { useStyles } from "./ForestCoverAreaYear.elements";
...
const ForestCoverAreaYear = ({ id, data, inputs, outputs }) => {
	...
	return (
		<div className={classes.node}>
			...
			<div className={classes.body}>
				...
				>
					<TreeItem
						nodeId={`${treeForestCoverArea.id}-${treeForestCoverArea.year.id}`}
						label={treeForestCoverArea.year.name}
					>
						{treeForestCoverArea.year.children.map((item) => (
							<TreeItem
								nodeId={`${treeForestCoverArea.id}-${treeForestCoverArea.year.id}-${item.id}`}
								label={item.name}
								key={item.id}
							/>
						))}
...
export default ForestCoverAreaYear;
