...
import { useStyles } from "./ForestCoverAreaCity.elements";
...
const ForestCoverAreaCity = ({ id, data, inputs, outputs }) => {
	...
	return (
		<div className={classes.node}>
			...
			<div className={classes.body}>
				...
				>
					<TreeItem
						nodeId={`${treeForestCoverArea.id}-${treeForestCoverArea.city.id}`}
						label={treeForestCoverArea.city.name}
					>
						{treeForestCoverArea.city.children.map((item) => (
							<TreeItem
								nodeId={`${treeForestCoverArea.id}-${treeForestCoverArea.city.id}-${item.id}`}
								label={item.name}
								key={item.id}
							/>
						))}
...
export default ForestCoverAreaCity;
