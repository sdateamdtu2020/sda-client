...
import { useStyles } from "./PopulationYear.elements";
...
const PopulationYear = ({ id, data, inputs, outputs }) => {
	...
	return (
		<div className={classes.node}>
			...
			<div className={classes.body}>
				...
				>
					<TreeItem
						nodeId={`${treePopulation.id}-${treePopulation.year.id}`}
						label={treePopulation.year.name}
					>
						{treePopulation.year.children.map((item) => (
							<TreeItem
								nodeId={`${treePopulation.id}-${treePopulation.year.id}-${item.id}`}
								label={item.name}
								key={item.id}
							/>
						))}
