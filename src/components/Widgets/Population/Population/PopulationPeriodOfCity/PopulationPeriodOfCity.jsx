...
import { useStyles } from "./PopulationPeriodOfCity.elements";
import {
	IconButton,
	InputLabel,
	MenuItem,
...
const PopulationPeriodOfCity = ({ id, data, inputs, outputs }) => {
	...
	useEffect(() => {
		if (city !== "" && fromYear !== "" && toYear !== "" && warning === "") {
			...
			dispatch(action);
			const itemIsSelect = [
				`${POPULATION}-${POPULATION_PRODUCTION}-${PERIOD_OF_CITY}`,
			];
			action = setItemIsSelectCity(itemIsSelect);
			dispatch(action);

			const idMerge = `${POPULATION}-${POPULATION_PRODUCTION}-${city}-${fromYear}-${toYear}`;
			...
			<div className={classes.body}>
				...
				<FormControl required className={classes.formControl}>
					...
					>
						...
						{treePopulation.year.children.map((item) => (
							<MenuItem key={item.id} value={item.id}>
								{item.name}
							</MenuItem>
						))}
...
export default PopulationPeriodOfCity;
